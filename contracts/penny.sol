...
/**
 *Penny Auction House - Main contract
 *Deployed at: 0x721E40573191deCF1c6b4fb78a7904d02d87D96C (Metis Stardust)
*/
...
contract AuctionHouse is ERC721TokenReceiver, ERC1155TokenReceiver {
    ...
    /// @param paymentToken Address of the token being used as payment method, use address(0) for payment in native tokens (Metis,etc.)
    ...
    function createAuction(address tokenContract, address paymentToken, uint itemId, uint auctionType,uint startPrice) public returns (uint) { 
    ...
    function claim(uint auctionId) public {
        ...
        if (al.bidCount > 0) {
            ...
            //Release the funds to auctioneer: 
             if (al.paymentToken == address(0)) {
                 al.auctioneer.transfer(al.totalRaised);
             } else { 
                 CErc20 payment = CErc20(al.paymentToken);
                 payment.transfer(al.auctioneer, al.totalRaised);
            }
